...
/**
 * Google Drive linking plugin for connecting a Google Drive account to an existing user.
 * Builds consent URL, handles callback to persist encrypted tokens, and
 * supports token refresh and access token retrieval.
 * 
 * This provider is specifically for Google Drive access with appropriate scopes.
 */
@Injectable()
export class GoogleDriveLinking implements LinkingProvider {
  ...
  /**
   * Build the Google consent URL for linking Google Drive to a user.
   * @param params.userId - Target application user id (signed into state)
   * @param params.scopes - Optional list of scopes, defaults to Google Drive access.
  ...
  buildLinkUrl(params: { userId: string; scopes?: string[]; mobile?: boolean }): string {
    ...
    const redirectUri = this.config.get<string>('GOOGLE_DRIVE_REDIRECT_URI') || this.config.get<string>('GOOGLE_REDIRECT_URI');
    ...
    const state = this.jwt.sign({ provider: 'google_drive', mode: 'link', userId: params.userId, mobile: params.mobile }, { expiresIn: '10m' });
  ...
  /**
   * Refresh the Google Drive access token using the stored refresh token.
  ...
  async refreshAccessToken(userId: string): Promise<{ accessToken: string; expiresIn: number }> {
    const account = await this.store.getLinkedAccount(userId, 'google_drive');
    if (!account || !account.refresh_token) throw new BadRequestException('No Google Drive refresh token stored');
    ...
    await this.store.updateLinkedTokens(userId, 'google_drive', {
  ...
  /**
   * Retrieve and decrypt the current Google Drive access token for the user.
   */
  async getCurrentAccessToken(userId: string): Promise<string> {
    const account = await this.store.getLinkedAccount(userId, 'google_drive');
    if (!account || !account.access_token) throw new BadRequestException('No Google Drive access token stored');
